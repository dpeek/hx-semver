...
typedef NativeFile = sys.io.File;
...
typedef NativeFile = neko.io.File;
...
typedef NativeFile = cpp.io.File;
...
class File
{
	...
	*/
	...
	/**
	Returns the time at which a file or directory was last modified. When path 
	is a directory, last modified is the most recent value of the directory 
	contents.

	@param path 	path to a file or directory
	@return last time path was modified 
	*/
	public static function lastModified(path:String):Float
	{
		Console.assert(path != null, "the argument 'path' cannot be null");
		path = File.nativePath(path);

		Console.assert(exists(path), "the path '" + path + "' does not exist");

		if (isDirectory(path))
		{
			var maxTime = 0.0;

			for (subPath in new Glob(path + "/*"))
			{
				var time = lastModified(subPath);
	...
	/**
	Determines if the source was modified more recently than target.
	@param source 	string path to a file or directory
	@param target 	string path to another file or directory
	*/
	public static function outdates(source:String, target:String):Bool
	{
		source = File.nativePath(source);
		target = File.nativePath(target);

		Console.assert(exists(source), "the path '" + source + "' does not exist");
		Console.assert(exists(target), "the path '" + target + "' does not exist");
		return lastModified(source) > lastModified(target);
	}
	...
	*/
	public static function size(path:String):Int
	{
		...
		{
			var total = 0;
			for (file in new Glob(path + "/**/*")) total += size(file);
			return total;
		}
	...
	*/
	public static function copy(fromPath:String, toPath:String)
	{
		...
		else
		{
			...
			NativeFile.copy(fromPath, toPath);
		}
	}
	...
	*/
	public static function read(path:String):String
	{
		...
		return NativeFile.getContent(path);
	}
	...
	*/
	public static function write(path:String, content:String, ?writeMode:FileWriteMode=null)
	{
		...
		Console.assert(directory == "" || isDirectory(directory), "the directory '" + directory + "' does not exist.");
		...
		switch(writeMode)
		{
			case FileWriteMode.overwrite:
				out = NativeFile.write(path, true);
			case FileWriteMode.append:
				out = NativeFile.append(path, true);
		}
